...
func (c *FClient) processLogin(p *protocols.Protocol) bool {
	...
	if s2cLogin.Status == 1 {
		...
		// 进入房间
		var c2sJoinRoom protocols.FruitC2SJoinRoom
...
func (c *FClient) processPlayerInfo(p *protocols.Protocol) bool {
	var s2cPlayer protocols.FruitS2CPlayerInfo
	s2cPlayer.Parse(p)
...
func (c *FClient) processJoinRoom(p *protocols.Protocol) bool {
	var s2cJoinRoom protocols.FruitS2CJoinRoom
	s2cJoinRoom.Parse(p)
...
func (c *FClient) processPlayResult(p *protocols.Protocol) bool {
	var s2cPlay protocols.FruitS2CPlayResult
	s2cPlay.Parse(p)
	if s2cPlay.Result == 1 {
		// 下注成功，返回游戏结果
...
func (c *FClient) play() {
	...
	// 发游戏协议
	c2sPlay := protocols.FruitC2SPlay{}
